...
var CLASSIC_GAMETRAILER_CANVAS_CONTEXT = null;
var CLASSIC_GAMETRAILER_DIRECTION = 3;
var CLASSIC_GAMETRAILER_POSITION_X = 600;
var CLASSIC_GAMETRAILER_POSITION_Y = 25;
var CLASSIC_GAMETRAILER_POSITION_STEP = 3;
var CLASSIC_GAMETRAILER_MOUNTH_STATE = 3;
var CLASSIC_GAMETRAILER_MOUNTH_STATE_MAX = 6;
var CLASSIC_GAMETRAILER_SIZE = 16;
...
function initHome() { 
	...
	var ctx = null;
	var canvas = document.getElementById('canvas-home-title-classic-game');
...
function startTrailer() { 
	...
	canvas.setAttribute('height', '50');
	if (canvas.getContext) { 
		CLASSIC_GAMETRAILER_CANVAS_CONTEXT = canvas.getContext('2d');
	}
...
function nextSequenceTrailer() { 
	...
	eraseClassicGameTrailer();
	eraseGhostsTrailer();
	
	if (CLASSIC_GAMETRAILER_MOUNTH_STATE < CLASSIC_GAMETRAILER_MOUNTH_STATE_MAX) { 
		CLASSIC_GAMETRAILER_MOUNTH_STATE ++; 
	} else { 
		CLASSIC_GAMETRAILER_MOUNTH_STATE = 0; 
	}
	if ( CLASSIC_GAMETRAILER_DIRECTION === 1 ) { 
		CLASSIC_GAMETRAILER_POSITION_X += CLASSIC_GAMETRAILER_POSITION_STEP;
	} else if ( CLASSIC_GAMETRAILER_DIRECTION === 3 ) { 
		CLASSIC_GAMETRAILER_POSITION_X -= CLASSIC_GAMETRAILER_POSITION_STEP;
	}
	if ( CLASSIC_GAMETRAILER_POSITION_X < -650) { 
		CLASSIC_GAMETRAILER_DIRECTION = 1;
		CLASSIC_GAMETRAILER_POSITION_STEP ++;
	}
...
function getGhostsTrailerCanevasContext() { 
	return CLASSIC_GAMETRAILER_CANVAS_CONTEXT;
}
...
function getClassicGameTrailerCanevasContext() { 
	return CLASSIC_GAMETRAILER_CANVAS_CONTEXT;
}
function drawClassicGameTrailer() { 

	var ctx = getClassicGameTrailerCanevasContext();
	...
	var lineToX = CLASSIC_GAMETRAILER_POSITION_X;
	var lineToY = CLASSIC_GAMETRAILER_POSITION_Y;
	if (CLASSIC_GAMETRAILER_DIRECTION === 1) { 
		startAngle = (0.35 - (CLASSIC_GAMETRAILER_MOUNTH_STATE * 0.05)) * Math.PI;
		endAngle = (1.65 + (CLASSIC_GAMETRAILER_MOUNTH_STATE * 0.05)) * Math.PI;
		lineToX -= 8;
	} else if (CLASSIC_GAMETRAILER_DIRECTION === 2) { 
		startAngle = (0.85 - (CLASSIC_GAMETRAILER_MOUNTH_STATE * 0.05)) * Math.PI;
		endAngle = (0.15 + (CLASSIC_GAMETRAILER_MOUNTH_STATE * 0.05)) * Math.PI;
		lineToY -= 8;
	} else if (CLASSIC_GAMETRAILER_DIRECTION === 3) { 
		startAngle = (1.35 - (CLASSIC_GAMETRAILER_MOUNTH_STATE * 0.05)) * Math.PI;
		endAngle = (0.65 + (CLASSIC_GAMETRAILER_MOUNTH_STATE * 0.05)) * Math.PI;
		lineToX += 8;
	} else if (CLASSIC_GAMETRAILER_DIRECTION === 4) { 
		startAngle = (1.85 - (CLASSIC_GAMETRAILER_MOUNTH_STATE * 0.05)) * Math.PI;
		endAngle = (1.15 + (CLASSIC_GAMETRAILER_MOUNTH_STATE * 0.05)) * Math.PI;
		lineToY += 8;
	}
	ctx.arc(CLASSIC_GAMETRAILER_POSITION_X, CLASSIC_GAMETRAILER_POSITION_Y, CLASSIC_GAMETRAILER_SIZE, startAngle, endAngle, false);
	ctx.lineTo(lineToX, lineToY);
	ctx.fill();
	ctx.closePath();
}
function eraseClassicGameTrailer() { 

	var ctx = getClassicGameTrailerCanevasContext();
	ctx.clearRect(CLASSIC_GAMETRAILER_POSITION_X - CLASSIC_GAMETRAILER_SIZE, CLASSIC_GAMETRAILER_POSITION_Y - CLASSIC_GAMETRAILER_SIZE, CLASSIC_GAMETRAILER_SIZE * 2, CLASSIC_GAMETRAILER_SIZE * 2);
}