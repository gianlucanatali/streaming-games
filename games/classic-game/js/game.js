...
function initGame(newGame) {
	...
	var player = localStoreManager.getUsername();

	loadHighestScore(localStoreManager.getGameName(), localStoreManager.getUsername(), this, function(hs,ctx) {
		//var localHighscore = ctx.storageManager.getBestScore();
		highScoreOnksqlDB = hs || 0;
		HIGHSCORE = highScoreOnksqlDB?highScoreOnksqlDB:HIGHSCORE;
		localStoreManager.setBestScore(HIGHSCORE);
	...
	// highScoreWorker = new Worker("/js/highscore-worker.js");
...
function doInitGame(newGame, lastScore, lastLevel) {
	...
	if (newGame) { 
		...
		var ctx = null;
		var canvas = document.getElementById('canvas-panel-title-classic-game');
...
function lifes(l) { 
	...
	var record = {};
	record.user = localStoreManager.getUsername();;
	record.game = {}
	record.game.score = SCORE
	record.game.lives = LIFES
	record.game.level = LEVEL

	produceToUserGame(localStoreManager.getUsername(),localStoreManager.getGameName(), SCORE, LIFES, LEVEL);
...
function gameover() { 
	...
	produceToUserLosses(localStoreManager.getUsername(),localStoreManager.getGameName());
