	...
	if (eval('GHOST_' + ghost.toUpperCase() + '_MOVING === false')) { 
		...
		if (canMoveGhost(ghost)) { 
			...
			if (eval('GHOST_' + ghost.toUpperCase() + '_BODY_STATE === 3') && eval('GHOST_' + ghost.toUpperCase() + '_STATE != -1')) { 
				if ( !CLASSIC_GAMEMOVING ) { 
					testGhostClassicGame(ghost);
				}
...
function changeDirection(ghost) { 
	...
	if (state === 0 || state === 1) { 
		if (ghostX != 276 && ghostY != 258) { 
			var classicGameX = CLASSIC_GAMEPOSITION_X;
			var classicGameY = CLASSIC_GAMEPOSITION_Y;
			var axe = oneAxe();
			if (ghost === "blinky") { 
			
				var nothing = whatsYourProblem();
				if (nothing < 6) { 
					tryDirection = getRightDirection(axe, ghostX, ghostY, classicGameX, classicGameY);
...
function getRightDirection(axe, ghostX, ghostY, classicGameX, classicGameY) { 
	if (axe === 1) { 
		if (ghostX > classicGameX) { 
		 return 3;
		} else { 
			return 1;
		}
	} else { 
		if (ghostY > classicGameY) { 
		 return 4;
		} else { 
			return 2;
		}
